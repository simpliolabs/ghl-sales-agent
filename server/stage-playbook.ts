...
const PLAYBOOKS: Record<string, StagePlaybook> = {
  "New Lead": {
    ...
    preferredFrameworks: ["HORMOZI_ACA", "CURIOSITY_HOOK", "SOCIAL_PROOF", "VALUE_FIRST", "PAS"],
    taskContext: null,
  ...
  "Contacted": {
    ...
    preferredFrameworks: ["DIRECT_RESPONSE", "VALUE_FIRST", "CASE_STUDY", "SOCIAL_PROOF", "HORMOZI_ACA"],
    taskContext: null,
  ...
  "Quote Sent": {
    ...
    preferredFrameworks: ["DIRECT_RESPONSE", "VALUE_FIRST", "SOCIAL_PROOF", "CASE_STUDY", "HORMOZI_INDIRECT"],
    taskContext: null,
  ...
  "Delivered": {
    stage: "Delivered",
    label: "Delivered",
    goal: "Turn this one-time buyer into a repeat customer. Use a structured 4-step post-delivery escalation: (1) satisfaction + review, (2) specific upsell/cross-sell with a concrete product hook, (3) seasonal/event trigger with urgency, (4) reactivation with a specific offer. NEVER send passive 'let me know if you need anything' messages.",
    focusTopics: [
      "Step 1 (first touch): Confirm satisfaction + ask for Google review with direct link",
      "Step 2 (7-14 days): Specific cross-sell or upsell — suggest a CONCRETE new product related to their order (e.g., 'We just started doing embroidered hats — want me to mock one up with your logo?')",
      "Step 3 (30+ days): Seasonal/event trigger with urgency — reference an upcoming season, holiday, or event and tie it to their business/organization (e.g., 'Summer's 6 weeks out — need gear for any events?')",
      "Step 4 (60+ days): Reactivation with a specific offer — 'Repeat customers get priority production + we still have your design on file. Want to reorder or try something new?'",
      "Always suggest a SPECIFIC product or service — never generic 'we're here if you need us'",
      "Reference their ORIGINAL order specifically (product type, event, business name)",
      "Ask for referrals with a concrete hook: 'Know anyone else planning [similar event/need]?'",
    ],
    neverDo: [
      "Send passive messages like 'let me know if you need anything' or 'we're always here'",
      "Send generic check-ins without a specific product suggestion or offer",
      "Forget to reference their original order (product, event, business name)",
      "Use vague language like 'your next group event' without specifying what product to suggest",
      "Hard-sell or pressure — the hook should be a specific, helpful suggestion, not a pitch",
      "Ignore complaints about the delivered order",
      "Send the same type of message twice — escalate through the 4-step sequence",
      "Be pushy about reviews — ask once, naturally, in Step 1 only",
    ],
    advanceSignals: [
      "Customer leaves a review",
      "Customer mentions a future order or event",
      "Customer refers someone",
      "Customer asks about a new product or reorder",
    ],
    escalationSignals: [
    ...
    tone: "Grateful, warm, and proactively helpful. Like a friend who just finished a project for you and already has ideas for the next one. Confident suggestions, not passive availability.",
    suggestedApproaches: ["post_delivery", "value_add", "seasonal", "reactivation", "relationship_nurture"],
    preferredFrameworks: ["EMB_POST_PURCHASE", "SOCIAL_PROOF", "VALUE_FIRST", "CASE_STUDY", "BAB", "HORMOZI_INDIRECT"],
    taskContext: null,
    noteTemplate: "Order delivered. AI following 4-step post-delivery escalation: satisfaction → upsell → seasonal → reactivation.",
    nextStage: null,
    followUpDelayHours: 72,
    aiProactive: true,
    isTerminal: true,
